...
class EfficientDetONNXDetector:
    ...
    def _apply_nms(self, boxes, scores, score_threshold=0.3, nms_threshold=0.3):
        """Apply Non-Maximum Suppression - Fixed for single detections"""
        # Use higher confidence threshold to reduce false positives
        score_threshold = max(score_threshold, 0.4)
        ...
        # More aggressive NMS for cleaner results
        keep = []
        while order.size > 0:
            i = order[0]
            keep.append(i)
            ...
            iou = intersection / (area_i + area_remaining - intersection + 1e-8)
            
            # Keep only boxes with low IoU (more aggressive suppression)
            keep_indices = np.where(iou <= nms_threshold)[0]
