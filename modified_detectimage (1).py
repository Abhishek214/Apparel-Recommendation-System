...
class EfficientDetONNXDetector:
    ...
    def _postprocess_detections(self, regression, classification, metadata, 
        ...
        regression = regression[0]  # Shape: [num_anchors, 4]
        classification = classification[0]  # Shape: [num_anchors, num_classes]
        ...
        for class_idx, class_name in enumerate(self.classes):
            ...
            current_thresh = sig_thresh if class_name == 'signature' else thresh
            
            # Filter by score threshold FIRST
            valid_mask = class_scores > current_thresh
            if not np.any(valid_mask):
                continue
                
            valid_scores = class_scores[valid_mask]
            valid_regression = regression[valid_mask]
            valid_anchors = self.anchors[valid_mask]
            
            # Convert regression to boxes
            dx, dy, dw, dh = valid_regression[:, 0], valid_regression[:, 1], valid_regression[:, 2], valid_regression[:, 3]
            
            # Anchor format: [x1, y1, x2, y2] normalized
            anchor_cx = (valid_anchors[:, 0] + valid_anchors[:, 2]) / 2
            anchor_cy = (valid_anchors[:, 1] + valid_anchors[:, 3]) / 2
            anchor_w = valid_anchors[:, 2] - valid_anchors[:, 0]
            anchor_h = valid_anchors[:, 3] - valid_anchors[:, 1]
            ...
            # Apply NMS
            keep_indices = self._apply_nms_cv2(pred_boxes, valid_scores, nms_threshold=0.5)
            
            # Keep only top detection per class
            if len(keep_indices) > 0:
                best_idx = keep_indices[0]  # Highest score after NMS
    ...
    def _apply_nms_cv2(self, boxes, scores, nms_threshold=0.5):
        """Simple NMS using sorting"""
        if len(boxes) == 0:
            return []
