...
def train(train_loader, model, scheduler, optimizer, epoch, args):
    ...
    for idx, (images, annotations) in enumerate(train_loader):
        images = images.float()  # Removed .cuda()
        annotations = annotations  # Removed .cuda()
...
def main_worker(args):
    ...
    # Model stays on CPU - no .cuda() calls
...
def main():
    ...
    if args.seed is not None:
        ...
        warnings.warn('You have chosen to seed training. '
                      'This will turn on deterministic behavior, '
                      'which can slow down your training considerably!')

    # Simply call main_worker function with CPU
    main_worker(args)
