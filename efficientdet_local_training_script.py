#!/usr/bin/env python3
"""
EfficientDet Training Script - Local Model Only
No external downloads, uses local .pth model file

Usage:
    python train_efficientdet_local.py --config configs/efficientdet_d1.yaml --pretrained_model /path/to/model.pth
"""
...
class SimpleEfficientDet(nn.Module):
    """Simplified EfficientDet implementation for local training"""
    ...
    def __init__(self, num_classes=4, image_size=640):
        ...
        # Simple backbone (can be replaced with your loaded model's backbone)
...
class COCODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for ann in anns:
            ...
            x, y, w, h = bbox
            x1 = x / img_info['width'] * self.image_size
            y1 = y / img_info['height'] * self.image_size
            x2 = (x + w) / img_info['width'] * self.image_size
            y2 = (y + h) / img_info['height'] * self.image_size
        ...
        target = {}
        if boxes:
            target['boxes'] = torch.tensor(boxes[0], dtype=torch.float32)  # Take first box for simplicity
            target['labels'] = torch.tensor(labels[0], dtype=torch.long)    # Take first label
        else:
            target['boxes'] = torch.zeros(4, dtype=torch.float32)
            target['labels'] = torch.tensor(0, dtype=torch.long)
            ...
            if state_dict:
                try:
                    ...
                    for k, v in state_dict.items():
                        ...
                        # Only load weights for layers that exist and have matching shapes
                        if key in model_dict and model_dict[key].shape == v.shape:
                            pretrained_dict[key] = v
                        else:
                            print(f"Skipping layer {key}: shape mismatch or not found")
...
def main():
    parser = argparse.ArgumentParser(description='Train EfficientDet with local model file')
